...
<?php foreach ($comments as $comment) : ?>
	<? if ($comment->comment_type != "trackback" && $comment->comment_type != "pingback" && !ereg("<pingback />", $comment->comment_content) && !ereg("<trackback />", $comment->comment_content)) { ?>
		...
		<?php comment_text() ?>
		<p><cite>
<?php comment_author_link() ?> &#8212; <?php comment_date() ?> @ <a href="#comment-<?php comment_ID() ?>"><?php comment_time() ?></a></cite> <?php edit_comment_link(__("Edit This"), ' |'); ?></p>
		</li>
...
<p><?php _e("Have a question? Know an explanation that caused your own a-ha moment? Write about it here.") ?> <!--<code><?php echo allowed_tags(); ?></code>--></p>
...
<form action="<?php echo get_settings('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
	...
	<p>
	  <label for="comment"><?php _e('Your Comment (HTML a and code tags allowed)'); ?></label>
	<br />
